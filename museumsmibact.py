...
url = "http://dbunico20.beniculturali.it/DBUnicoManagerWeb/dbunicomanager/searchPlace?modulo=luoghi&tipologiaLuogo=1&stato=P&quantita=100&offset=5"
...
step = 0
print idx
url = "http://dbunico20.beniculturali.it/DBUnicoManagerWeb/dbunicomanager/searchPlace?modulo=luoghi&tipologiaLuogo=1&stato=P&offset=%s" % (idx[0]) 
#url = "http://dbunico20.beniculturali.it/DBUnicoManagerWeb/dbunicomanager/searchPlace?modulo=luoghi&tipologiaLuogo=1&stato=P&offset=0&quantita=50" 

xml = urllib2.urlopen(url)
docxml = etree.parse(xml).findall("mibac")
with open('musei.csv', 'wb') as csvfile:
    museiwriter = csv.writer(csvfile, delimiter=';', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    museiwriter.writerow(["nome","indirizzo","comune","provincia","sitoweb","latitudine","longitudine"])
    for mibac in docxml:
        print step
        writerow = []
        metainfo = mibac.find('metainfo').getchildren()
        stato = metainfo[0].getchildren()[0].text
        enteCompilatore = metainfo[0].getchildren()[1].text
        nomeRedattore = metainfo[0].getchildren()[2].text
        nomeCapoRedattore = metainfo[0].getchildren()[3].text
        dataValidazione = metainfo[0].getchildren()[4].text
        dataUltimaModifica  = metainfo[0].getchildren()[5].text
        datacreazionexml = metainfo[1].text
        sorgente = metainfo[2].text
        ...
        denominazione = luogodellacultura[4].getchildren()
        ...
        step += 1
        if (longitudine != ""):
            writerow.append(nome)        
            writerow.append(via_indirizzo)
            writerow.append(comune_indirizzo)
            writerow.append(provincia_indirizzo)
            writerow.append(sitoweb)
            writerow.append(latitudine)
            writerow.append(longitudine)
